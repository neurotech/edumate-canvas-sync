...
WITH staff_evergreen_courses AS (
  ...
  UNION
  SELECT 'LEARN-ED' AS "COURSE_ID", 'STAFF' AS "USER_TYPE" FROM SYSIBM.SYSDUMMY1
),
...
parent_evergreen_courses AS (
  ...
  UNION
  SELECT 'PASTORALCARE-PARENTS' AS "COURSE_ID", 'PARENTS' AS "USER_TYPE" FROM SYSIBM.SYSDUMMY1
),
...
staff_non_ms3_pastoral_care_enrolments AS (
  SELECT
    current_staff.contact_id || '.' || staff.staff_number || '.' || staff.staff_id AS "USER_ID",
    (CASE
      WHEN class.class LIKE '%MS1' THEN '2017-PC-YR08'
      WHEN class.class LIKE '%MS2' THEN '2017-PC-YR09'
      WHEN class.class LIKE '%MS4' THEN '2017-PC-YR07'
      WHEN class.class LIKE '%SS1' THEN '2017-PC-YR11'
      WHEN class.class LIKE '%SS2' THEN '2017-PC-YR12'
      WHEN class.class LIKE '%SS3' THEN '2017-PC-YR10'
    END) AS "course_id",
    'STAFF' AS "USER_TYPE"
