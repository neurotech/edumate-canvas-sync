...
edumate.queryHandler = function(dataset, sql, output) {
  return new Promise(function(resolve, reject) {
...
// Query the database
edumate.query = function(dataset, sql, output) {
  return new Promise(function(resolve, reject) {
    acho.warning('\'' + dataset + '\'' + ' query initiated.');
    acho.info('output destination: ' + output);
    // init db
    edumate.init()
      .then(function(res) {
        acho.success(res);
        ...
        edumate.open()
          .then(function(conn) {
            acho.success(conn);
            // query db
            edumate.queryHandler(dataset, sql, output)
